import sys
from write5f import Data_Analyser
from read5f import Read_Data
...
def Token_val(R_W, Content_Type, accessid_token, data):
    try:
        ...
        if flag == "0":
            ...
            results = Data_Analyser(data, flag, Content_Type)
        elif flag == "1":
...
def determine_escalation1():
    ...
    ###############user-host-name
    host = '*'
    with open("host_name.txt", "w") as write_file:
