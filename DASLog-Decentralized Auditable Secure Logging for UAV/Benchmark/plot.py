...
plt.plot(xnew, f_cubic(xnew), '--o', label='Simple-DASLog', linewidth=3)
...
plt.plot(xnew1, f_cubic1(xnew1), '--^',label='DASLog', linewidth=3)
...
df6 = pd.read_excel('tree_sheets_Bsig.xlsx', 'df_6')
xnew2 = np.linspace(df2['C'].min(), df2['C'].max(), 8)
f_cubic2 = interp1d(df2['C'], df2['B'], kind='cubic')
plt.plot(xnew2, f_cubic2(xnew2), '--o', label='Simple-DASLog: Hashchain+Merkle tree', linewidth=3)
plt.scatter(xnew2, f_cubic2(xnew2), s=100)
xnew5 = np.linspace(df5['C'].min(), df5['C'].max(), 8)
f_cubic5 = interp1d(df5['C'], df5['Bhc'], kind='cubic')
plt.plot(xnew5, f_cubic5(xnew5), '--^', label='Simple-DASLog: Hashchain', linewidth=3)
plt.scatter(xnew5, f_cubic5(xnew5), s=100, marker='^')
xnew6 = np.linspace(df6['C'].min(), df6['C'].max(), 8)
f_cubic6 = interp1d(df6['C'], df6['B'], kind='cubic')
plt.plot(xnew6, f_cubic6(xnew6), '--P', label='DASLog: Signature+Merkle tree', linewidth=3)
plt.scatter(xnew6, f_cubic6(xnew6), s=100, marker='P')
plt.grid()
plt.xticks(np.arange(0,10001,1000))
